"""Optimize the weakest v25 hard-cardinality certificate with GPU PDLP."""
...
def main() -> None:
    ...
    hard_ids = set(record["lagrangian_certificate"]["hard_cardinality_cut_ids"])
    hard_cuts = tuple(
        cut
        for cut in cuts
        if cut.cut_id in hard_ids and isinstance(cut, CommitmentCardinalityCut)
    )
    if len(hard_cuts) != len(hard_ids):
        raise RuntimeError("Hard-cardinality certificate identity is incomplete")
    current = replay_lagrangian_certificate(
        master,
    ...
    for row_name, value in current.coupling_duals:
        row_dual[row_by_name[row_name]] = value
    supplied_by_id = dict(current.commitment_cut_duals)
    cut_dual = np.asarray(
        [supplied_by_id.get(cut.cut_id, 0.0) for cut in cuts],
        dtype=np.float64,
    )
        ...
        + json.dumps(
            {
                ...
                "initial_exact_bound": initial_bound,
                "final_exact_bound": current.conservative_lower_bound,
