#!/usr/bin/env python3
"""Probe a GPU PDLP multiplier-search LP on v13 bottleneck leaves."""
...
def main() -> None:
    ...
    for frontier_record in bottlenecks:
        ...
        column_scale, _row_scale = native_scaling_vectors(
            search.canonical,
        ...
        solve = solve_cuopt_continuous_pdlp(
            ...
            presolve=0,
            initial_native_primal=search.initial_values / column_scale,
        ...
        if solve.values is None or not np.all(np.isfinite(solve.values)):
            raise RuntimeError(f"Multiplier-search PDLP returned no vector for {region_id}")
        candidate_row_dual = inherited_row_dual.copy()
        ...
        for position, column in zip(
            ...
            search.coupling_columns,
            strict=True,
        ):
            coupling = coupling_rows[int(position)]
            value = float(solve.values[int(column)])
            candidate_row_dual[coupling.row_index] = (
                min(value, 0.0)
                if coupling.kind != "balance_equality"
                else value
            )
        candidate_cut_dual = np.minimum(
            np.asarray(
                solve.values[search.commitment_cut_columns], dtype=np.float64
            ),
            0.0,
        )
        ...
        if candidate.conservative_lower_bound >= inherited.conservative_lower_bound:
            selected = candidate
            selected_source = "gpu_multiplier_search_lp"
        else:
            selected = inherited
            selected_source = "inherited_monotone_fallback"
        serialized = selected.as_dict(source_rows, compact=True)
        ...
        replay_difference = abs(
            replayed.conservative_lower_bound - selected.conservative_lower_bound
        )
        if replay_difference > float(
            config.raw["benchmark"]["gpu_cpu_replay_tolerance_dollars"]
        ):
            raise RuntimeError(f"Multiplier-search certificate failed replay for {region_id}")
        records.append(
            {
                "region_id": region_id,
                ...
                "selected_conservative_lower_bound": selected.conservative_lower_bound,
                "selected_source": selected_source,
                "improvement_dollars": (
                    selected.conservative_lower_bound
                ...
                "search_solver_budget_seconds": 30.0,
    ...
    output = {
        "diagnostic_id": "activsg2000-v18-dual-lp-probe",
        "status": "passed",
    ...
    output_path = (
        root / "results" / "diagnostics" / "activsg2000-v18-dual-lp-probe.json"
    )
